function [BFdata_aligned,TRITICdata_Aligned] = A3_3_alignBFandTRITIC(BFdata,TRITICdata,SaveFigFolder,idxMon)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%% ALIGN BF-TRITIC IMAGES PRE AND POST AFM (for normal AFM scans) %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    Image_BF_PRE=BFdata.pre;
    % TRITIC images with highest optical parameters
    ...
    while true
        if ~flagAlign_BFWithTRITIC && flag_BF_prepost
            ...
            for i=1:numel(fnames)
                th_TRITICall=TRITICdata.(fnames{i});
                % apply custom function to any cell
                if isequal(fnames{i},'post')                
                    tmp=cellfun(@(x) fixSize(x,-offset),th_TRITICall,'UniformOutput',false);
                else
                    % minus because post images was fixed during alignment with pre image
                     tmp=cellfun(@(x) fixSize(x,offset),th_TRITICall,'UniformOutput',false);
                end
                TRITICdata_Aligned.(fnames{i})=tmp;
            end
        end
        if flagAlign_BFWithTRITIC
            % Otherwise keep the original method (i.e. two separate alignments) ==================>>>>>>>>>>>>>>>>>>>> PART NOT COMPLETED
            %       TRITIC_After WITH TRITIC_Before
            %       BF with TRITIC_Before
            % Align the fluorescent images After with the BEFORE stimulation
            [Image_TRITIC_POST,Image_TRITIC_PRE,offset]=alignBinarizedImages(Image_TRITIC_POST,Image_TRITIC_PRE,idxMon);        %#ok<*ASGLU>
            Image_BF_PRE=fixSize(Image_BF_PRE,offset); %#ok<NASGU>
            % fix TRITICpost to BFpre
            error("THIS PART HASNT BEEN CHECKED SINCE THE PREVIOS CONDITION WORKS VERY WELL")
            TRITICdata_Aligned=""; %#ok<UNRCH>
        end
        ...
        elseif getValidAnswer(sprintf('Is the registration of TRITICpre and TRITICpost postAlign ok?'),'',{'Yes','No'})
            close all
            BFdata_aligned.pre=Image_BF_PRE_aligned;
            if flag_BF_prepost
                BFdata_aligned.post=Image_BF_POST_aligned;
            end
            break
        else            
            flagAlign_BFWithTRITIC=true;        
        end
        close all
    end    
