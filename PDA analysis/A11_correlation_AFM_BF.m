function varargout=A11_correlation_AFM_BF(AFM_data,AFM_IO_Padded,setpoints,secondMonitorMain,newFolder,varargin)
   ...
   if innerBord 
    ...
    if ~flag_heat
        ...
        % all lateral force
        dataPlot_Height_LD=A11_feature_CDiB(AFM_data(idx_H).Padded_masked(:),AFM_data(idx_LD).AFM_Padded(:),secondMonitorMain,newFolder,'NumberOfBins',numBins,'xpar',1e9,'ypar',1e9,'YAyL','Lateral Force (nN)','XAxL',sprintf('Feature height (nm)'),'FigTitle','Height VS Lateral Deflection','NumFig',1);
        ...
        dataPlot_Height_LD_maxVD=A11_feature_CDiB(AFM_data(idx_H).Padded_masked(:),AFM_data(idx_LD).Padded_masked_maxVD(:),secondMonitorMain,newFolder,'NumberOfBins',numBins,'xpar',1e9,'ypar',1e9,'YAyL','Lateral Force (nN)','XAxL',sprintf('Feature height (nm)'),'FigTitle','Height VS Lateral Deflection (up to max vertical force)','NumFig',2);
    ...
    if ~flag_onlyAFM
        ...
        if ~flag_heat
            ...
            % all lateral force
            dataPlot_LD_FLUO_padMask=A11_feature_CDiB(AFM_data(idx_LD).Padded_masked(:),Delta_ADJ(:),secondMonitorMain,newFolder,'NumberOfBins',2500,'xpar',1e9,'ypar',1,'YAyL','Absolute fluorescence increase (A.U.)','XAxL','Lateral Force (nN)','FigTitle','LD (only PDA) Vs Fluorescence increase','NumFig',4);
            varargout{4}=dataPlot_LD_FLUO_padMask;
            % lateral force up to max vertical force
            dataPlot_LD_FLUO_padMask_maxVD=A11_feature_CDiB(AFM_data(idx_LD).Padded_masked_maxVD(:),Delta_ADJ(:),secondMonitorMain,newFolder,'NumberOfBins',500,'xpar',1e9,'ypar',1,'YAyL','Absolute fluorescence increase (A.U.)','XAxL','Lateral Force (nN)','FigTitle','LD (only PDA and up to the max vertical force) Vs Fluorescence increase','NumFig',5);
            varargout{5}=dataPlot_LD_FLUO_padMask_maxVD;
           
            % VERTICAL FORCE VS FLUORESCENCE INCREASE
            dataPlot_VD_FLUO=A11_feature_CDiB(AFM_data(idx_VD).Padded_masked(:),Delta_ADJ(:),secondMonitorMain,newFolder,'setpoints',setpoints,'xpar',1e9,'ypar',1,'YAyL','Absolute fluorescence increase (A.U.)','XAxL','(Measured) Vertical Force (nN)','FigTitle','VD Vs Fluorescence increase','NumFig',6); 
    ...
    if ~flag_heat
        ...
        dataPlot_VD_LD=A11_feature_CDiB(AFM_data(idx_VD).Padded_masked(:),AFM_data(idx_LD).Padded_masked(:),secondMonitorMain,newFolder,'setpoints',setpoints,'xpar',1e9,'ypar',1e9,'YAyL','Lateral Force (nN)','XAxL','(Measured) Vertical Force (nN)','FigTitle','LD Vs VD','NumFig',7);
